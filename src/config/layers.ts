 ...
 */
export type LayerBasis = 'score' | 'window' | 'bivariate';

/** One axis of a bivariate layer. */
export interface BivariateAxis {
  /** Legend axis label. Must state the direction, or the map lies. */
  label: string;
  /** The end of the axis the colour saturates toward. */
  highLabel: string;
}

export interface LayerDef {
  key: LayerKey;
  /** Metric-toggle / card title, matching the wireframe. */
  label: string;
  /** Formal name for the map panel header (§6.5). */
  formalName: string;
  /**
   * Layer-specific copy for the panel.
   *
  ...
  /** Defaults to 'score'. */
  basis?: LayerBasis;
  /** Window layers only: the monthly County series the loss is measured on. */
  seriesField?: 'snap_enrolled' | 'snap_children';
  /** Legend heading in count measure, where `countField` does not name it. */
  countLabel?: string;
  /** Bivariate layers only: the red axis then the blue axis. */
...
function basisOf(l: LayerDef): LayerBasis {
  return l.basis ?? 'score';
}

export const layerBasis = (key: LayerKey): LayerBasis => basisOf(layerByKey(key));
 ...
 */
export function scoreFieldOrNull(key: LayerKey): keyof import('../types').County | null {
...
export function hasCountBasis(key: LayerKey): boolean {
  const l = layerByKey(key);
  return basisOf(l) === 'window' || l.countField != null;
}

/** Legend heading in count measure. */
export function countLabelFor(key: LayerKey): string {
  const l = layerByKey(key);
...
export interface TooltipSpec {
  /**
   * The other domains' bands, two-up under the focused one.
   *
   ...
   */
  domains: boolean;
  /** The four listed-assistance registries. Capacity is the index view's story. */
  capacity: boolean;
  /**
   * The chart at the foot of the panel.
   *
   *   'enrollment'  the caseload sparkline across the window
   *   'exposure'    caseload against newly-subject against who actually left
   *   'none'        no chart
   */
  chart: 'enrollment' | 'exposure' | 'none';
}

export interface ViewDef {
  key: ViewKey;
  /** Strip card title. Plain language — the strip is read before the panel. */
  label: string;
  /** One line under the card title, so the strip explains itself. */
  blurb: string;
  /**
   * The layers this view can map. First is the default; a second entry raises a
  ...
  metrics: LayerKey[];
  /**
   * Whether assistance-capacity marks, the mark key and the gap overlay draw.
   *
  ...
  /** What the county tooltip shows here. */
  tooltip: TooltipSpec;
}
...
export const viewByKey = (key: ViewKey): ViewDef =>
 ...
 */
export function viewForLayer(layer: LayerKey): ViewDef {
  return VIEWS.find((v) => v.metrics.includes(layer)) ?? VIEWS[0];
}

/** The layer a view opens on. */
export const defaultLayerFor = (key: ViewKey): LayerKey => viewByKey(key).metrics[0];
