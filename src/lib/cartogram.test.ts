...
import { dougenikCartogram, ringsToPath, type CartogramRegion, type Ring } from './cartogram';

/** A grid of unit squares, so areas and adjacency are trivial to reason about. */
function grid(cols: number, rows: number, size = 10): CartogramRegion[] {
  const out: CartogramRegion[] = [];
...
const areaOf = (r: CartogramRegion) => r.rings.reduce((s, ring) => s + ringArea(ring), 0);
