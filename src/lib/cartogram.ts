...
const areaOf = (rings: Ring[]) => rings.reduce((s, r) => s + ringArea(r), 0);

export interface CartogramOptions {
  /** More iterations converge closer at linear cost. */
  iterations?: number;
  /**
   * How far to move from the true map toward the fully converged cartogram,
...
export interface CartogramResult {
  regions: CartogramRegion[];
  /** Mean absolute area error after the final pass, as a fraction. 0 = perfect. */
  error: number;
  iterations: number;
}
...
export function dougenikCartogram(
  ...
  { iterations = 4, blend = 0.75 }: CartogramOptions = {},
): CartogramResult {
  // Work on a copy; the input geometry is shared with the geographic views.
  let current: CartogramRegion[] = regions.map((r) => ({
    id: r.id,
    rings: r.rings.map((ring) => ring.map((p) => [p[0], p[1]] as [number, number])),
  }));
    ...
    current = current.map((region, ri) => ({
      ...
      rings: region.rings.map((ring, gi) =>
        ring.map(([x, y], vi) => {
          ...
          return [ox + (x - ox) * t, oy + (y - oy) * t] as [number, number];
        }),
      ),
    }));
  }
