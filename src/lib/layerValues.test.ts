...
describe('windowed loss layers', () => {
  ...
  it('shows a missing series as no data, not as zero loss', () => {
    const missing = [county('z', [1000, 900], undefined as unknown as number[])];
  ...
  it('keeps its contrast when the window changes length', () => {
    ...
    const short = [county('a', [1000, 900]  as number[], [1, 1]), county('b', [1000, 975], [1, 1])];
    const long = [county('a', [1000, 800] as number[], [1, 1]), county('b', [1000, 950], [1, 1])];
...
describe('the bivariate work layer', () => {
  ...
  it('is the only layer that hands down a second axis', () => {
    for (const layer of ['loss', 'child_loss', 'd1', 'd3', 'composite'] as const) {
...
describe('per-view tooltip specs', () => {
  const spec = (key: Parameters<typeof viewForLayer>[0]) => viewForLayer(key).tooltip;

  it('shows the domain spread on the vulnerability index and nowhere else', () => {
    expect(spec('composite').domains).toBe(true);
    for (const layer of ['loss', 'child_loss', 'd1', 'd3', 'work_both'] as const) {
