...
import type { LayerKey, LossKey } from '../types';
 ...
 */
export const LOSS_LAYERS: readonly LossKey[] = ['loss', 'child_loss'];

const isLoss = (layer: LayerKey): boolean => (LOSS_LAYERS as readonly string[]).includes(layer);
 ...
 */
export const LOSS_BAND_MULTIPLES = [0, 0.6, 0.8, 0.95, 1.1, 1.3] as const;
 ...
 */
export function binsForLoss(statewideShare: number): readonly number[] {
...
export function colorFor(
  ...
  bins: readonly number[] = binsFor(layer),
): string {
 ...
 */
export function bivariatePalette(steps: number = BIVARIATE_STEPS): string[][] {
 ...
 */
export function quantileBins(values: number[], classes: number): readonly number[] {
