 ...
 */
import type { County, Statewide } from '../types';

export interface Scope {
  /** 'state', or a county geoid. */
  key: string;
  /** "Texas" or "Harris County". */
  label: string;
  /** Resident population — the participation-rate denominator. */
  population: number;
  /** Monthly enrolled individuals, parallel to meta.months. */
  enrolled: number[];
  /** Monthly enrolled children, or null where the series is missing. */
  children: number[] | null;
  /** null when scoped to the state. */
  county: County | null;
}
...
export function monthlyChange(series: number[]): (number | null)[] {
 ...
 * shape.
 */
export function indexTo(series: number[], refIndex: number): (number | null)[] {
...
export function participationShare(series: number[], population: number): number[] {
...
export interface WindowChange {
  ...
  absolute: number;
  /** Signed fraction of the start value. Negative is a loss. */
  pct: number | null;
}

export function windowChange(series: number[], i0: number, i1: number): WindowChange {
...
export interface LossRank {
  /** 1 = steepest percentage loss in Texas. */
  rank: number;
  total: number;
  /** The county's own signed percentage change. */
  pct: number;
  /** The statewide figure, for the comparison sentence. */
  statePct: number;
  /** Median county percentage change — the typical county, not the total. */
  medianPct: number;
}
...
export function lossRank(
  ...
  const rows = counties
    ...
    .filter((r): r is { geoid: string; pct: number } => r.pct != null)
...
export interface FunnelPoint {
  geoid: string;
  name: string;
  /** Caseload at the window start — the precision axis. */
  caseload: number;
...
export interface FunnelData {
  points: FunnelPoint[];
  /** Statewide percentage change — the funnel's centre line. */
  center: number;
  /** Control limits, widening as caseload shrinks. */
  limits: { caseload: number; sd: number }[];
}
...
export function buildFunnel(
  ...
  const sdAt = (caseload: number): number => {
...
export function funnelBand(
  ...
  maxCaseload: number,
  steps = 60,
): { caseload: number; lo: number; hi: number }[] {
