...
/**
 * @typedef {Object} ColumnChartProps
 * @property {string[]} months
 * @property {(number | null)[]} values
 * @property {Annotation[]} [annotations]
 * @property {number} [height]
 * @property {boolean} [labelEveryMonth]
 * @property {number} [viewWidth] - viewBox width. Keep it near the rendered CSS width so one SVG unit is about
...
export function ColumnChart({
  ...
  const x = (i) => M.left + i * bandWidth;
  const y = (v) => M.top + (1 - (v + extent) / (2 * extent)) * (height - M.top - M.bottom);
  const zero = y(0);
