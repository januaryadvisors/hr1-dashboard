...
export interface ExposureRow {
  label: string;
  value: number;
  /** Share of the caseload, printed after the count. Omitted on the baseline. */
  share?: number | null;
  tone: 'base' | 'subject' | 'left';
  /** §10: the newly-subject estimate is PUMS-modelled, not a lookup. */
  modelled?: boolean;
}

export interface ExposureBarsProps {
  rows: ExposureRow[];
}

export function ExposureBars({ rows }: ExposureBarsProps) {
